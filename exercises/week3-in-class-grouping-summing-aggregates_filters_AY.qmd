# Week 3 In Class Grouping and Summing - Aggregates & Filters {#r-aggregates_filters}
...
Show the neighborhoods between the second quartile and the median of income in 2020

```{r}
mydata <- baltcity_income %>%
  select(Neighborhood, x2020) %>% 
...
baltcity_race %>% 
  ...
  summarize(
    ...
    Pac_Islander = sum(x2020_pac_islander, na.rm = TRUE)
  )
```
...
```{r}
mean(baltcity_income$x2020, na.rm = TRUE)
```
