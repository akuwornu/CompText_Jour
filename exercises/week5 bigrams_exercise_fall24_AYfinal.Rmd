...
```{r}
Whitepress_Articles <- lynch %>%  #also known as the non-black press
   filter(!is.na(newspaper_name)) %>% 
   count(newspaper_name) %>%               
  arrange(desc(n))
print(Whitepress_Articles)
```
...
The bigrams from non-black press showed significantly higher values than the black press bigrams. That correlated with the volume of non-black press relative to the black press. In effect, the era of the study was characterized by more non-black press and it was also during the period of segregation. It was thus common to see the use of words such as negro, lynch, mob, jail, lynching and negroes in a lot of non-black press. There could be many explanations for this, but presummably, the black press must have reported such words with anger or disdain while the white press must have preferred the use of such words. But at least, the persistent use of such words gives an indication of the times of these newspaper coverages. Admittedly, the collation of the bigrams was quite difficult. I am still trying to grasp the principles. I believe one more session on this will clarify a few confusions I had.
