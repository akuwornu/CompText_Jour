...
range(MediaBucks$Salary, na.rm = TRUE)
```
```{r}
#Range of Salary data
MediaBucks %>% 
 select(Salary) %>% 
summarize(range = sum(Salary, na.rm=TRUE))
```
...
```{r}
#Number of rows cut with filter greater than 500
nrow(MediaBucks %>% 
       filter(Salary > 500))
```
...
MediaSalary %>% 
  select(Gender, Salary) %>% 
  group_by(Gender) %>% 
  summarize(Total = range(Salary, na.rm=TRUE))
```
<br>
...
NYT <- subset(MediaBucks, COMPANY=="NewYorkTimes" | COMPANY =="NYT") 
```
...
#Which one is higher?
#Bloomberg is higher than Business
```
...
```{r}
Business <- subset(MediaBucks, grepl ("?business", TITLE, ignore.case = TRUE))
summary(Business$Salary, na.rm= TRUE)
...
```{r}
#2: Make a table for editors, figure out medians.
Editor <- subset(MediaBucks, grepl ("?editor", TITLE, ignore.case = TRUE))
summary(Editor$Salary, na.rm= TRUE)
